...
class AmlJob():
    ...
    def __init__(self,
        ...
        self.env_name = 'research_copilot_env_acr'
    ...
    def create_or_get_environment(self):
        try:
            ...
            # Use the SAME docker image as the API app (ACR, Python 3.10, all deps preinstalled).
            # Environment.from_pip_requirements() defaults to AML's curated Python 3.8 base image,
            # which breaks with modern SDKs that use Python 3.9+ type syntax
            # (e.g. azure-search-documents: "TypeError: 'type' object is not subscriptable").
            # NOTE: DOCKER_REGISTRY_SERVER_URL is NOT injected as an env var when the app pulls
            # images with a managed identity — use the AZURE_CONTAINER_REGISTRY app setting.
            registry = (os.getenv('AZURE_CONTAINER_REGISTRY') or os.getenv('DOCKER_REGISTRY_SERVER_URL') or '').replace('https://', '').rstrip('/')
            if registry:
                env = Environment.from_docker_image(self.env_name, f'{registry}/research-copilot-api:latest')
                env.register(self.ws)
                self.env = self.ws.environments[self.env_name]
                logging.info(f'Creating new environment {self.env_name} from docker image {registry}/research-copilot-api:latest.')
            else:
                env = Environment.from_pip_requirements(self.env_name, 'requirements.txt')
                env.register(self.ws)
                self.env = self.ws.environments[self.env_name]
                logging.info(f'Creating new environment {self.env_name} from pip requirements.')
