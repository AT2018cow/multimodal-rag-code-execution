...
def get_models():
    try:
        ...
        # Never return API keys to the client. The UIs only use this list to
        # count the available model pools (by AZURE_OPENAI_RESOURCE), so keys
        # are masked here.
        masked_models = []
        for m in gpt4_models:
            masked = dict(m)
            masked['AZURE_OPENAI_KEY'] = '***'
    ...
    try:
        ...
        def run_search_in_thread(input: SearchRequest, request_steps_queue: asyncio.Queue):
            ...
            try:
                ...
                request_steps_queue.put_nowait(("ERROR", str(e)))
                raise HTTPException(status_code=500, detail=str(e))
            finally:
