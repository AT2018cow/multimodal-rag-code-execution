...
def extract_doc_using_doc_int(ingestion_pipeline_dict):
    ...
    # api_version MUST be passed explicitly: the pinned SDK (azure-ai-documentintelligence 1.0.0b4)
    # defaults to the retired 2024-02-29-preview, which the service now rejects with (Gone).
    document_intelligence_client = DocumentIntelligenceClient(endpoint=DI_ENDPOINT, credential=get_entra_credential() if use_entra_auth() else AzureKeyCredential(DI_KEY), api_version=DI_API_VERSION or None)
