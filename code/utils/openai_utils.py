...
from env_vars import TENACITY_STOP_AFTER_DELAY, TENACITY_TIMEOUT, AZURE_OPENAI_VISION_API_VERSION, AZURE_VISION_ENDPOINT, AZURE_VISION_KEY, get_openai_endpoint, use_entra_auth, get_entra_credential, get_entra_token, COGNITIVE_SERVICES_SCOPE, search_auth_headers
...
OPENAI_API_BASE = get_openai_endpoint(os.getenv('AZURE_OPENAI_RESOURCE')).rstrip('/') + '/'
AZURE_OPENAI_MODEL = os.environ.get('AZURE_OPENAI_MODEL')

AZURE_OPENAI_EMBEDDING_MODEL= os.environ.get('AZURE_OPENAI_EMBEDDING_MODEL')
AZURE_OPENAI_EMBEDDING_API_BASE = get_openai_endpoint(os.getenv('AZURE_OPENAI_EMBEDDING_MODEL_RESOURCE')).rstrip('/')
...
if use_entra_auth():
    oai_client = AzureOpenAI(
        ...
        azure_ad_token_provider = lambda: get_entra_token(COGNITIVE_SERVICES_SCOPE),
...
def get_chat_completion(messages: List[dict], model = AZURE_OPENAI_MODEL, client = oai_client, temperature = 0.2):
    ...
    # temperature intentionally not sent: reasoning models (gpt-5.6-sol) reject any non-default temperature
    return client.chat.completions.create(model = model, messages = messages, timeout=TENACITY_TIMEOUT)
...
def get_chat_completion_with_json(messages: List[dict], model=AZURE_OPENAI_MODEL, client=oai_client, temperature=0.2):
    try:
        ...
        # temperature intentionally not sent: reasoning models (gpt-5.6-sol) reject any non-default temperature
        return client.chat.completions.create(model=model, messages=messages, response_format={"type": "json_object"}, timeout=TENACITY_TIMEOUT)
...
def ask_LLM(prompt, temperature = 0.2, model_info = None):
    ...
    if model_info is not None:
        if use_entra_auth():
            client = AzureOpenAI(
                azure_endpoint =  get_openai_endpoint(model_info['AZURE_OPENAI_RESOURCE']) ,
                azure_ad_token_provider = lambda: get_entra_token(COGNITIVE_SERVICES_SCOPE),
                api_version= AZURE_OPENAI_API_VERSION,
            )
        else:
...
def call_gpt4v(imgs, gpt4v_prompt = "describe the attached image", prompt_extension = "", temperature = 0.2, model_info=None, enable_ai_enhancements=False):
    ...
    if model_info is None:
        ...
        api_base = get_openai_endpoint(model_info['AZURE_OPENAI_RESOURCE']).rstrip('/') + '/'
    ...
    if use_entra_auth():
        headers["Authorization"] = "Bearer " + get_entra_token(COGNITIVE_SERVICES_SCOPE)
    else:
