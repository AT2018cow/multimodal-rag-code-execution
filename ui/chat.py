...
class APIClient:
    ...
    def get_prompts(self):
        try:
            response = api_auth_session.get(f"{self.base_url}/prompt")
            response.raise_for_status()
            return response.json()
        except requests.exceptions.HTTPError as e:
    ...
    def get_prompt(self, p):
        try:
            response = api_auth_session.get(f'{self.base_url}/prompt/{p}')
            response.raise_for_status()
            return response.json()
        except requests.exceptions.HTTPError as e:
    ...
    #     try:
    #         response = api_auth_session.post(f"{self.base_url}/ingest", json=ingestion_params)
    #         response.raise_for_status()
    #         return response.json()
    #     except requests.exceptions.HTTPError as e:
    ...
    def get_file(self, asset_path):
        try:
            response = api_auth_session.get(f"{self.base_url}/file", params={'asset_path': asset_path})
            response.raise_for_status()
    ...
    def check_file_exists(self, asset_path):
        try:
            response = api_auth_session.get(f"{self.base_url}/file_exists", params={'asset_path': asset_path})
            response.raise_for_status()
            return response.json()
        except requests.exceptions.HTTPError as e:
    ...
    def search(self, query_params):
        try:
            response = api_auth_session.post(f"{self.base_url}/search", json=query_params)
            response.raise_for_status()            
            return response.json()
        except requests.exceptions.HTTPError as e:
    ...
    async def search_stream(self, query_params):
        try:
            # httpx does not go through api_auth_session — attach the token explicitly
            headers = {}
            token = api_auth.get_api_token()
            if token:
                headers["Authorization"] = f"Bearer {token}"
            async with httpx.AsyncClient() as client:
                async with client.stream("POST", f"{self.base_url}/search-stream", json=query_params, headers=headers, timeout=3 * 60.0) as response:
